...
# Alunos: Caio Antunes, Felipe Spitale, Pedro Santos
# Matrículas (respec.): 22153136, 22154141, 22060035
...
def get_best_categories(conn):
    try:
        with conn.cursor() as cursor:
            cursor.execute("""
                SELECT subquery.group, subquery.salesrank, subquery.titulo
                FROM (
                    SELECT 
                        P.titulo, 
                        P.salesrank, 
                        P.group,
                        ROW_NUMBER() OVER(PARTITION BY P.group ORDER BY P.salesrank) AS rank
                    FROM amazonproducts.produto AS P
                ) AS subquery
                WHERE subquery.rank <= 10 and subquery.salesrank != -1
                ORDER BY subquery.group, subquery.salesrank ASC;
...
def top10_comments_pergroup(conn):
    try:
        with conn.cursor() as cursor:
            cursor.execute("""
                        SELECT ClienteId, Grupo, Total_Comentarios
                        FROM (
                            SELECT 
                                ClienteId, 
                                P.group AS Grupo, 
